...
- 二重登録・二重承認防止、手修正保護、再試行、資料と派生情報の削除
- 暗号化された本文・候補・要約・タスク、認可とログの機密情報抑制
- 架空のサンプルによる操作体験（AIの代替や実データの解析ではありません）

PDF・画像、見積比較、決定事項、回答待ち、夫婦の別アカウント共有は次段階です。現在はテキストからタスクを作る最小フローです。
...
タスク移行の使い方は `docs/task-import.md` を参照してください。「やること → データを取り込む」から準備済みのJSONを選び、担当を割り当てて登録します。ゲスト・収支・BGMと.xlsx直接アップロードは未対応です。
