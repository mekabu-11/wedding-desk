...
- 手動タスク追加、開始日、状態・担当・カテゴリ・期間・文字検索、週／月ガント、一括変更
...
- 今週・期限超過・出欠・検討中・見込収支・未入出金・最近の変更をまとめた概要
...
Excel移行は「やること → Excelをまとめて移行」から2つのxlsxを選び、統合プレビューで行別警告と担当A/Bを確認して反映します。上限は各25MB、展開後100MB、合計10,000行です。数式・外部リンク・マクロは実行せず、実績額や日付不明の入出金は確定・入出金記録にしません。従来のJSONタスク取り込みは `docs/task-import.md` の導線から引き続き利用できます。
