...
- Docker内の `bin/verify`：**113 runs / 922 assertions / 0 failures / 0 errors / 0 skips**。`zeitwerk:check`、`git diff --check` も成功。
- ホストRuby（2.6.10）はGemfileのRuby 3.4指定と不一致のため、Docker外のRailsテストは実行していない。検証はプロジェクトのDocker環境で実施した。

未実装の設計候補は、ご祝儀の属性別フォールバックと食事セットである。これらは既存の世帯ごとのご祝儀区分・引き出物を壊さない別段階として残している。
...
- 費用と検討項目の自動作成費用を扱うルールは今回変更していない。候補変更時の自動除外や `source_kind` の追加は未実装のまま、既存の手動リンク操作だけを記録する。
